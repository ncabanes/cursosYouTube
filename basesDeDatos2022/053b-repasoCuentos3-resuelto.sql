...
SELECT nombre, anyoNacimiento
FROM autores
WHERE anyoNacimiento IS NOT NULL
ORDER BY anyoNacimiento ASC
...
SELECT nombre, anyoNacimiento
FROM autores
WHERE anyoNacimiento = 
(
	SELECT MIN(anyoNacimiento)
	FROM autores
	WHERE anyoNacimiento IS NOT NULL
);
...
SELECT nombre, anyoNacimiento
FROM autores
WHERE anyoNacimiento <= ALL 
(
	SELECT anyoNacimiento
	FROM autores
	WHERE anyoNacimiento IS NOT NULL
);
...
(
	SELECT anyoNacimiento
	FROM autores a2
	WHERE a2.anyoNacimiento < a1.anyoNacimiento
)
AND anyoNacimiento IS NOT NULL;
...
(
	...
	WHERE paises.nombre = cuentos.titulo
);
